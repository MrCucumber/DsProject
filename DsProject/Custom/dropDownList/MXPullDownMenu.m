...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete
{
    if (show) {
        ...
        tableView.frame = CGRectMake(87, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(87, self.frame.origin.y + self.frame.size.height, self.frame.size.width, tableViewHeight);
        }];

    } else {
        
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(87, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
        } completion:^(BOOL finished) {
